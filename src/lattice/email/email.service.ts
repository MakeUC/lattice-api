...
export class EmailService {
  ...
  verificationTemplate: HandlebarsTemplateDelegate<{
    fullName: string;
  ...
  getverificationEmailTemplate(): void {
    readFile(
      `${__dirname}/../../../templates/verification.html`,
      (err, data) => {
        if (err) Logger.error(err);
        if (!data)
          Logger.error(new Error(`Verification email template not found`));
  ...
  async sendVerificationEmail(registrant: Registrant): Promise<void> {
    ...
    sgMail
      ...
      .then(() => {
        Logger.log(
          `Verification email sent to ${registrant.email} successfully`,
        );
      })
      .catch((err) => {
        Logger.error(
          `Verification email could not be sent to ${registrant.email}: ${err.message}`,
        );
      });
  }
  ...
  async sendWelcomeEmail(registrant: Registrant): Promise<void> {
    ...
    sgMail
      ...
      .catch((err) => {
        Logger.error(
          `Welcome email could not be sent to ${registrant.email}: ${err.message}`,
        );
      });
  }
