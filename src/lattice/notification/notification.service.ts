...
export class NotificationService {
  ...
  private async createNotification(
    ...
    const pushSubscriptions = await this.subscriptions.find({
    ...
    pushSubscriptions?.forEach(async (ps) => {
      try {
        ...
        Logger.error(
          `Could not push notification to ${from.email}: ${err.message}`,
        );
      }
    });
  ...
  private async hydrateNotification(
    ...
    const to = await this.users.findOne(notification.to, {
      select: [
        `email`,
        `name`,
        `skills`,
  ...
  async subscribe(
    ...
    const pushSubscription = this.subscriptions.create({
      userId,
      subscription,
    });
